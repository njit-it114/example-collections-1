...
public class SortPerson1 {
	...
	/**
	 * @param n number of times to add p into returned list
	 * @param p optional instance to populate the returned list with. If null, new Person("",0) is used
	...
	public static List<Person> createPersons(int n, Person p) {
		...
		List<Person> myPersons = new ArrayList<Person>(n);	// NOTE this is still an empty List with initial capacity of personsOrig size
...
//		personsOrig = SortPerson1.createPersons();
		personsOrig = createPersons();	// NOTE this line is equivalent to above one. Although above one is recommended, this line is OK in the same class.
		persons = personsOrig;
	}
...
//		persons = personsOrig;	// WRONG to do shallow copy, instead do deep copy like below
...
//		persons = personsOrig;	// WRONG to do shallow copy, instead do deep copy like below
	...
	public List<Person> sortPersonBasedOnReverseAge() {
		...
		Person.PersonAgeComparator personAgeComparator = new Person.PersonAgeComparator();
		Comparator reverseAgeComparator = Collections.reverseOrder(personAgeComparator);
